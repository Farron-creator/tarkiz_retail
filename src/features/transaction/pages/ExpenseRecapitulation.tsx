...
import { ExpensesSummary, ExpensesSummaryQuery, TransactionStatus } from '../types';

export const ExpenseRecapitulation: React.FC = () => {
  const { outlet } = useOutletContext();
  const [params, setParams] = useState<ExpensesSummaryQuery>({
    outlet: outlet?.id,
    status: ['accepted'],
    startDate: new Date(),
    endDate: new Date(),
  ...
  const { data, isLoading, isError } = useExpensesSummary({ params });
  ...
  return (
    <main className="bg-gray-50">
      <Navbar title="Rekapitulasi Pengeluaran" withBorder to="/" />
