...
const ExpenseItem: React.FC<ItemProps> = ({ product, onChange }) => {
...
type Props = {
  ...
  onChange: (item: ExpenseRequest['items']) => void;
};

export const ExpenseItemList: React.FC<Props> = ({ products, items, onChange }) => {
  ...
  return (
    <div className="divide-y space-y-4 divide-gray-300">
      {products
        ...
        .map((product) => (
          <ExpenseItem key={product.id} product={product} onChange={handleChange(product)} />
        ))}
    </div>
  );
};
