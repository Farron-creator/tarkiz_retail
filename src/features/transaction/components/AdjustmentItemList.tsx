...
const AdjustmentItem: React.FC<ItemProps> = ({ product, onChange }) => {
...
type Props = {
  products: Product[];
  items: AdjustmentRequest['items'];
  onChange: (item: AdjustmentRequest['items']) => void;
};

export const AdjustmentItemList: React.FC<Props> = ({ products, items, onChange }) => {
  ...
  return (
    <div className="divide-y space-y-4 divide-gray-300">
      {products
        ...
        .map((product) => (
          <AdjustmentItem key={product.id} product={product} onChange={handleChange(product)} />
        ))}
    </div>
  );
};
