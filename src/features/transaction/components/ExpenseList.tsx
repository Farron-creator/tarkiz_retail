...
import { Expense, ExpenseQuery, TransactionStatus } from '../types';

import { ExpenseStatus } from './TransactionStatus';

const ExpenseItem: React.FC<Expense> = (expense) => {
  return (
    <Link
    ...
    >
      ...
      {expense.items.length > 0 && (
        <div className="text-xs text-gray-600 flex flex-col items-end">
          <ul className="m-0">
            <li>
              {expense.items[0].quantity} {expense.items[0].product.unit}{' '}
              {expense.items[0].product.name}
            </li>
          </ul>
          {expense.items.length > 1 && <IconDots stroke={0.9} size={14} />}
        </div>
      )}
    </Link>
  );
...
export const ExpenseList: React.FC<Props> = () => {
  ...
  const [params, setParams] = useState<ExpenseQuery>({
    outlet: outlet?.id,
    startDate: undefined,
    endDate: undefined,
    status: [],
  ...
  const expenses = data?.pages.reduce(
  ...
  return (
    <>
      ...
      {expenses?.length == 0 && <div className="px-5">Belum ada pembelian</div>}
